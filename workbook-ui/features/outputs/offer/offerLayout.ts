import offerTexts from '@/workbook-ui/data/offerTexts.json';
import type { InputValues, OfferResult } from '@/workbook-ui/engine';

const TEXTS = offerTexts as unknown as Record<string, { desc: string; unit: string }> & { footer: string[] };

/** Input keys whose sum is shown as the line quantity (workbook Piedāvājums column E). */
...
export function buildOfferLines(offer: OfferResult, inputs: InputValues): OfferLine[] {
  ...
  for (let line = 8; line <= 26; line++) {
    const amount = offer.lines[line];
    if (!amount) continue;
    out.push({
      line,
      description: TEXTS[String(line)].desc.replace(/\s+/g, ' ').trim(),
      specification: spec[line] ?? '',
      unit: TEXTS[String(line)].unit,
      quantity: QTY_KEYS[line] ? QTY_KEYS[line].reduce((a, k) => a + num(k), 0) : 1,
      amount,
    });
  }
  return out;
...
export const OFFER_FOOTER: string[] = TEXTS.footer.map((t) => String(t).replace(/^\d+\.\s*/, ''));
