...
export async function GET() {
  ...
  try { return NextResponse.json({ok: true, settings: await getEstimatorSettings()}); }
...
export async function PATCH(request: NextRequest) {
  ...
  try { return NextResponse.json({ok: true, settings: await saveEstimatorSettings((await request.json()).settings)}); }
