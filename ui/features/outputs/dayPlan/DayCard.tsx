...
import type { DayPlanDay } from '@/ui/api';
import { formatDate } from '@/ui/lib/format';

export interface DayTracking { crew?: string; hours?: string; done?: boolean; note?: string }
interface Props { day: DayPlanDay; tracking: DayTracking; onChange: (t: DayTracking) => void }

/** One day: planned tasks + on-site tracking fields (candidate for a `day_log` table). */
export function DayCard({ day, tracking, onChange }: Props) {
  ...
  return (
    <article className={day.isWeekend ? 'day-card is-weekend' : 'day-card'}>
      <header><strong>Day {day.dayNo}</strong><small>{formatDate(new Date(`${day.date}T00:00:00`), { weekday: 'short', day: 'numeric', month: 'short' })}</small></header>
